...
class ThrottleRequests
{
    ...
    //     );
    // }
    protected $decayMinutes = 2; //change the duration of the time window

    public function handle($request, Closure $next, $maxAttempts = 0, $decayMinutes = 0)
    {
        if($request->ip() === '127.0.0.1') //Whitelist IP address
        {
            return $next($request);
        }
        elseif($request->routeIs('some.route')) //Whitelist route
        {
            return $next($request);
        }

        return $this->handle($request, $next, $maxAttempts, $decayMinutes);
    }
